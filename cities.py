...
def cities_menu(cities):
    print("-" * 83)
    print(
        f"|{"city name":<20} | {"population":>20} | {"does the city has a station?":>35}|"
    )
    print("-" * 83)
    for city in cities.values():
        ...
        print(
            f"|{city["name"]:<20} | {str(city["population"]):>20} | {hasStation:<35}|"
        )
    print("-" * 83)
...
def new_city(cities):
    ...
    print(
        f"A new city with the name {name} has been founded! It has a population of {population} people"
    )
    return cities
...
def grow_city(cities):
    city = cities[
        str(random.randint(0, len(cities) - 1))
    ]  # Decide what city should grow
    city["population"] += random.randint(1000, 10000)
    print(
        f"The city {city["name"]} has grown! It now has a population of {city["population"]} people"
    )
    return cities
